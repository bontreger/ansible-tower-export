        ...
        if "encrypted" in data:
            ...
            for x in data:
                if not(isinstance(data[x], (bool))):
                    if "encrypted" in str(data[x]) and isinstance(data[x], str):
                        index_name = x
                        key = get_encryption_key("notification_configuration", secret, kid)
                        credential_object['notification_configuration'][index_name] = (decrypt_value(key, data[index_name]))
